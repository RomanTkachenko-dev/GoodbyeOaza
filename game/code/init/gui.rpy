...
label splashscreen:
    ...
    #show text 'Warning:\n JQuartz is not responsible for any laziness that may result from using main menu generator. JQuartz is also protected from being held responsible for global warming that may result from using too much computer. Please be advised.' at Position(xpos.5, ypos=.2) at Position(xpos=.5, xanchor=.5, ypos=.5, yanchor=.5)
    show text 'ПОПЕРЕДЖЕННЯ'
    $ ui.keymap(mouseup_1=ui.returns("_return"))
    $ ui.pausebehavior( 3, result="Skip")
    $ ui.interact()
    show text 'Гра містить звуки вибухів, нецензурну лексіку, зображення вживання наркотичних речовин, згадки про суїцид та депресію, а також образи, що можуть викликати судоми в людей зі світочутливістю та схільністю до епілепсії. Бережіть себе та проявляйте розсудливість.'
    $ ui.keymap(mouseup_1=ui.returns("_return"))
    $ ui.pausebehavior( 8, result="Skip")
    $ ui.interact()

    show text 'Ваші вибори мають значення та будуть впливати на сюжет. Але не завжди — все як у житті ;)'
    $ ui.keymap(mouseup_1=ui.returns("_return"))

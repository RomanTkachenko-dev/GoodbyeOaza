...
label proval2:
    ...
    "Я продовжую стояти, незнаючи, що мені робити.."
...
label brama:
    ...
    "Зупинка транспортерів направо від мене. Біля платформи як раз стоїть потрібний мені бус"

    "Його двері широко відчинені, а на платформі вже нікого немає"
    play sound "audio/Сцена 4/Звук автомобіля.mp3"
    "Якщо покваплюсь, ще встигну застрибнути"
    ...
    menu:
        "Оббігти і роздивитись":
            ...
            "Зайва обережність не завадить. Хто знає, що чекає на мене в середині?"
            "Швидкий огляд не виявляє жодної небезпеки"
            "Салон заповнений досить щільно, але місця ще є"
            "Пасажири розгублено дивляться по сторонах, намагаючись зрозуміти, що відбувається"
            play music "audio/Сцена 7/Тривога.mp3" fadeout 1
            гучномовець "ТРИВОГА! ТРИВОГА!"
            "Клятий гучномовець продовжує голосити"
            "Дуже скоро цю вулицю теж перекриють. Нема часу на зволікання"
            stop music
            play music "audio/Сцена 8/Тема Втечі.mp3"
            $ timer_jump = 'proval_brama'
            $ time = 5
            ...
            menu:
                "Застрибнути в салон":
                    hide screen countdown
                    "В останню мить переді мною в салон влітає якась тітка у фіолетовому балахоні"
                    "Я спокійно заходжу всередину, намагаючись не привертати уваги"

                    play sound "audio/Сцена 8/Закриття Трамвайних Дверей.mp3"

                    "Двері зачиняються з майже заспокійливим шурхотом"

                    play sound "audio/Сцена 8/Механічний Гуркіт Транспорту, Який Рушив З Місця.mp3"

                    "Транспортер з гуркотом вмикає двигуни і з поштовхом відривається від землі"
        ...
        "Бігти прямо у двері":
            ...
            "Здається, я не одна намагаюсь застрибнути в салон в останню хвилину"
            "В дверях транспортеру я стикаюсь з не по рокам спритною жіночкою. Таке враження, що вона просто матералізувалась в повітрі переді мною"

            play music "audio/Сцена 7/Тривога.mp3" fadeout 1

            гучномовець "ТРИВОГА! ТРИВОГА!"
            "Клятий гучномовець продовжує голосити"
            "Ми намагаємося одночасно пролізти в занадто вузькі двері"
            "В останній момент я хапаюсь за поручень і з силою проштовхую себе і її в салон"
            stop music
            play music "audio/Сцена 8/Тема Втечі.mp3"
            play sound "audio/Сцена 8/Звук Зіткнення Двох Людей.mp3"
            "..."
            ...
            "Ми обидві падаємо на підлогу. Двері транспортеру з шурхотом зачиняються за нами"
            пасажирка "От дурна периферійка! Понаїхало вас в наш район, навіть транспортером користуватися не вмієте!"
            "Обурена тітка підводиться, отряхує поли свого одягу і непевно крокує до вільного місця"
            "Вона явно сп'яніла від годіуму. Це пояснює підвищену спритність і дивний блиск в очах"
            "А також те, що вона не помітила склянку Г36, яка випала у неї з сумки і закотилась під одне з сидіннь"
            ...
            menu:
                "Непомітно взяти годіум собі":
                    hide screen countdown
                    $ втеча_годіум = True
                    "Я машинально хапаю пляшку і кладу собі в кишеню"
                    "Не те, щоб я хотіла його вживати. Але зважаючи на наявні обставини, така річ може стати в нагоді"
                    play sound "audio/Сцена 8/Механічний Гуркіт Транспорту, Який Рушив З Місця.mp3"
                    "Транспортер з гуркотом вмикає двигуни і з поштовхом відривається від землі"
                    "Там йому і місце. Слава невживанцям!"

                    play sound "audio/Сцена 8/Механічний Гуркіт Транспорту, Який Рушив З Місця.mp3"
                    "Транспортер з гуркотом вмикає двигуни і з поштовхом відривається від землі"
                    "Пляшка провалюється в щелину між сидіннями. Тепер її вже не дістати"
                    "Можливо, воно і на краще"
    ...
    "Поки бус набирає висоту, я займаю сидіння і озираюсь по сторонах"
    "Пасажири виглядають збентежено і розгублено"
    "Цей транспортер, як і будь-який інший, обладнаний мініатюрними трансконнекторами, вмонтованими в спинки сидінь"
    "Щоб втомлені пасажири могли отримати свою дозу дешевого щастя прямо по дорозі з роботи до дому"
    "Але зараз ці приблуди не працюють. Схоже, вірус Антаресу дістався і до них"
    "Що станеться з цим містом без годіуму? Важко уявити"
    "Транспортер повільно (занадто повільно!) летить над містом до південно-східної брами"

    scene втеча_без_машини with dissolve
    "Ми вже майже підлітаємо до брами, коли раптом…"
    play sound "audio/Сцена 8/Близький Вибух.mp3"
    "Гучний вибух, одразу за ним - потужний поштовх"
    "Транспортер хитає з боку в бік, наче билинку на вітру"
    play sound "audio/Сцена 8/Далекий Вибух.mp3"
    "Згодом слідує ще один вибух, цього разу більш далекий"
    "Я визираю у вікно і дивлюсь у напрямку брами"
    "Але не бачу її: вся споруда охоплена полум'ям і димом"
    "От і прилетіли…"
    "Якщо хтось і чекав на мене біля тої брами, то зараз точно вже не чекає"
    "Яка ж я була дурна, що повірила тому Лайї…"
    ...
    "Дідько! Це пастка!"
    "Транспортер знижується на найближчий дах. На ньому вже чекають дві металеві потвори"
    "Щойно бус приземляється, двері відчиняються і в них влітають двоє роботів-Перевіряючих"
    "Ще декілька секунд, і їх металеві щупальця схоплять мене. Що ж робити?"
